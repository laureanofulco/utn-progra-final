...
void menu_administracion(void)
{
	...
	do
	{
        ...
        printf("2 - Reporte ultimos artistas agregados\n");
...
void reporte_ultimos_artistas(void)
{
    ...
    while (fread(&art_aux, sizeof(Artista), 1, archivo) == 1)
    {
        if (art_aux.activo == 1)
        {
            apilar(&pila_ids, art_aux.id);
        }
    }
    ...
    printf("<---- ULTIMOS ARTISTAS ---->\n");

    if (pilavacia(&pila_ids))
    {
        printf("No hay artistas registrados.\n");
        return;
    }

    while (!pilavacia(&pila_ids))
    {
        int id_reciente = desapilar(&pila_ids);

        if (buscar_artista_id(id_reciente, &art_aux))
        {
            printf("ID: %d | Nombre: %s\n", art_aux.id, art_aux.nombre);
        }
    }
}